/* 5 by Peaceful World — cumulative practice time + free practice timer. */
'use strict';
...
(() => {
  ...
  const STRINGS = {
    ru: {
      ...
      title: 'Таймер',
      idea: 'Практикуйся столько, сколько нужно.',
      note: 'Без цели и без серии. Просто время, проведённое в практике.',
      start: 'Начать',
      pause: 'Пауза',
      resume: 'Продолжить',
      finish: 'Завершить',
    ...
    en: {
      ...
      title: 'Timer',
      idea: 'Practise for as long as you need.',
      note: 'No goal and no streak. Just time spent practising.',
      start: 'Start',
      pause: 'Pause',
      resume: 'Continue',
      finish: 'Finish',
  ...
  style.id = 'pw-free-practice-style';
  style.textContent = [
  ...
  function renderOdometer() {
    ...
    odometer.hidden = seconds <= 0 && count <= 0;
    odometer.querySelector('.pw-time-odometer-label').textContent = copy.totalLabel;
    odometer.querySelector('.pw-time-odometer-value').textContent = formatTotal(seconds);
  ...
  function nativeHistoryPush(screenId) {
    if (!window.PW_NATIVE_ANDROID) return;
    if (history.state?.pwScreen === screenId) return;
    history.pushState({ pwScreen:screenId }, '');
  }

  function nativeHistoryReplace(screenId) {
    if (!window.PW_NATIVE_ANDROID) return;
    history.replaceState({ pwScreen:screenId }, '');
  }

  function showFreeTimer() {
    document.querySelectorAll('.screen.active').forEach(node => node.classList.remove('active'));
    screen.classList.add('active');
    nativeHistoryPush(screen.id);
    renderCopy();
    renderTimer();
  }
  ...
  function startTimer() {
    ...
    if (firstStart) {
      playBoundarySignal();
      document.dispatchEvent(new CustomEvent('pw:practice-started', { detail:{ mode:'free' } }));
    }
  ...
  function pauseTimer() {
    ...
    document.dispatchEvent(new CustomEvent('pw:progress-changed'));
  }
  ...
  function finishTimer() {
    ...
    if (elapsedMs > 0) {
      ...
      document.dispatchEvent(new CustomEvent('pw:completion-changed'));
      playBoundarySignal();
    }
    elapsedMs = 0;
    creditedSeconds = 0;
